...
class Graph {
    public:
        /**
         * Generate a graph with random distances
         * 
        ...
        /**
         * Populate a graph from a file with given distances
         * 
         * @param path Path to the file.
         */
        Graph(string path) {
            ifstream graph_file(path);
            istream_iterator<int> itr(graph_file), end;
            ...
            while (itr != end) {
                std::vector<int> edge;
                for (int i = 0; i < 3 && itr != end; ++i, ++itr) {
                    edge.push_back(*itr);
                }
                if (edge.size() != 3) {
                    cerr << "Missing values in file.\n";
                    continue;
                }
